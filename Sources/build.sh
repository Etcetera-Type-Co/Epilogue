...
echo "Generating VFs"
fontmake -g Epilogue.glyphs -o variable --output-path ../fonts/variable/Epilogue[wght].ttf
fontmake -g Epilogue_italic.glyphs -o variable --output-path ../fonts/variable/Epilogue-Italic[wght].ttf
...
do
	...
	mv "$ttf.fix" $ttf;
	# woff2_compress $ttf this appears to cause issues in variable fonts https://github.com/google/woff2/issues/123
done
rm ../fonts/variable/*backup*.ttf

for ttf in ../fonts/variable/*.ttf
do
	gftools gen-stat $ttf --src stat.yaml --inplace;
done

python postprocessing.py
mv ../fonts/variable/Epilogue-Italic[wght].ttf.fix ../fonts/variable/Epilogue-Italic[wght].ttf
mv ../fonts/variable/Epilogue[wght].ttf.fix ../fonts/variable/Epilogue[wght].ttf


echo "Generating Static fonts"
fontmake -g Epilogue.glyphs -i -o ttf --output-dir ../fonts/static/ttf/
fontmake -g Epilogue_italic.glyphs -i -o ttf --output-dir ../fonts/static/ttf/

fontmake -g Epilogue.glyphs -i -o otf --output-dir ../fonts/static/otf/
fontmake -g Epilogue_italic.glyphs -i -o otf --output-dir ../fonts/static/otf/
